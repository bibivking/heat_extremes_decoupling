...
def main(fname1, fname2):
    ...
    for site in sites:
        ...
        if count == 0:
            sns.distplot(df_site.slope, ax=ax, rug=True, hist=True,
                         kde_kws={"label": "KDE"})
        else:

            sns.distplot(df_site.slope, hist=True, ax=ax, rug=True)
    ...
    ofname = "ozflux_histogram_Qle_ozflux.pdf"
...
if __name__ == "__main__":
    ...
    data_dir = "data/"
    fname1 = os.path.join(data_dir, "ozflux_slopes_Qle.csv")
    fname2 = os.path.join(data_dir, "ozflux_slopes_GPP.csv")
    main(fname1, fname2)
